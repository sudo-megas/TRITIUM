...
fun CurrencyAskDialog(onConfirm: (String) -> Unit) {
    // rememberSaveable, not remember: this dialog cannot be dismissed or
    // cancelled — it is the only thing on screen until answered — so a
    // configuration change (rotation) must not silently erase a selection
    // or a typed custom code and force the maker to notice and retype it.
    // AF12 audit finding.

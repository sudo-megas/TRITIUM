...
const rules = [
  ...
  {
    pattern: /\bnew\s+Intl\.[A-Za-z]+\s*\(\s*\)/,
    why: 'Intl constructor without an explicit locale'
  },
  {
    pattern: /\bnew\s+Intl\.[A-Za-z]+\s*\(\s*undefined/,
    why: 'Intl with undefined locale falls back to the system'
  },
  { pattern: /\btoLocaleString\s*\(\s*\)/, why: 'toLocaleString() without an explicit locale' },
  {
    pattern: /\btoLocaleDateString\s*\(\s*\)/,
    why: 'toLocaleDateString() without an explicit locale'
  },
  {
    pattern: /\btoLocaleTimeString\s*\(\s*\)/,
    why: 'toLocaleTimeString() without an explicit locale'
  },
  { pattern: /LanguageDetector/, why: 'i18next language detection is forbidden' },
  {
    pattern: /\bIntl\.\s*DateTimeFormat\s*\(\s*\)/,
    why: 'DateTimeFormat without an explicit locale'
  }
]
